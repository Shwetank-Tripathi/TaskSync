...
export async function handleGetRooms(req, res) {
  try {
    const rooms = await Room.find()
      .populate("members logs")
      .populate({ path: "tasks", populate: { path: "assignedUser" } });
...
export async function handleGetUserRooms(req, res) {
  try {
    const rooms = await Room.find({ members: req.user._id })
      .select("_id name createdBy members tasks")
...
export async function handleCreateRoom(req, res) {
  try {
    ...
    if (sanitizedName.length > 50) {
      return res
        .status(400)
        .json({ message: "Room name is too long (max 50 characters)" });
    }
...
export async function handleGetRoom(req, res) {
  try {
    const room = await Room.findById(req.params.id)
      ...
      .populate({
        path: "tasks",
        populate: { path: "assignedUser", select: "_id name email" },
      })
      .populate("logs");
  ...
  try {
    ...
    if (room.createdBy.toString() !== userId.toString()) {
      return res.status(403).json({
        message:
          "Only the room creator can delete this room. Use 'Leave Room' to exit as a member.",
      });
    }
    ...
    console.log(
      `🗑️ Room ${room.name} and all associated data deleted by creator ${userId}`,
    );
  ...
  try {
    ...
    if (room.createdBy.toString() === userId.toString()) {
      return res.status(400).json({
        message:
          "Room creators cannot leave their own room. Use 'Delete Room' to delete it entirely.",
      });
    }
